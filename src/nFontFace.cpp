...
namespace ngl{
  ...
  const Glyph& FontFace::get_glyph(char code){
    ...
    if( FT_Load_Char(m_ftFace, code, FT_LOAD_RENDER | FT_LOAD_FORCE_AUTOHINT) ){
    ...
    for(int y = 0; y < bitmap.rows; ++y){
      memcpy(pixels + y * bitmap.width, bitmap.buffer + (bitmap.rows - y - 1)* bitmap.pitch, bitmap.width);
    }
    ...
    glyph.off.x   =0;//(m_ftFace->glyph->metrics.horiBearingX >> 6) - (m_ftFace->glyph->metrics.width >> 6);
    glyph.off.y   =(m_ftFace->glyph->metrics.horiBearingY >> 6) - (m_ftFace->glyph->metrics.height >> 6);
  ...
  //--------------------------------------------------------------------------//
  /// \brief  Load the given font face.
  ...
  void FontFace::load(const String &face, size_t size){
    ...
    float k    =(float)m_ftFace->size->metrics.x_ppem / m_ftFace->units_per_EM;
